...
if echo "$EFFECTIVE" | grep -qi "^permitrootlogin yes"; then
  echo -e "[FAIL][CRITICAL] ${RED}Root login is enabled${NC}"
  fail=1
else
...
if echo "$EFFECTIVE" | grep -qi "^passwordauthentication yes"; then
  echo -e "[FAIL][HIGH] ${RED}SSH Password Authentication is enabled${NC}"
  fail=1
else
...
if echo "$EFFECTIVE" | grep -qi "^port 22$"; then
  echo -e "[FAIL][LOW] ${RED}SSH is running on default port 22${NC}"
  fail=1
else
